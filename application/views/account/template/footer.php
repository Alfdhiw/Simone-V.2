        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="modal-title" id="exampleModalLabel">Apakah Anda Yakin?</h2>
                    <button class="close" type="button" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">×</span>
                    </button>
                </div>
                <div class="modal-body text-secondary">Pilih tombol "Logout" dibawah ini untuk keluar dari akun saat ini</div>
...
<script src="<?= base_url(); ?>assets/js/home/datetime.js"></script>
...
$sql1 = "select status AS status,count(0) AS jumlah 
from absensi
where status = 1 and kode_magang = '$kode_magang' union 
select status AS status,count(0) AS jumlah 
from absensi  
where status = 2 and kode_magang = '$kode_magang'  union 
select status AS status, count(0) AS jumlah
from absensi a
where status = 0 and kode_magang = '$kode_magang';";
$result1 = mysqli_query($con, $sql1);
$chart_data = "";
while ($row1 = mysqli_fetch_array($result1)) {

    $tes[]  = $row1['status'];
    $jumlahtes[] = $row1['jumlah'];
}

?>
<script>
    var tes = <?= json_encode($tes); ?>

    // [7, 8, 8, 9, 9, 9, 10, 11, 14, 14, 15];
    var jumlahtes = <?= json_encode($jumlahtes); ?>
    // [7, 8, 8, 9, 9, 9, 10, 11, 14, 14, 15];
    var barColors = [
        "#b91d47",
        "#00aba9",
    ...
    new Chart("absenChart", {
        ...
        options: {
            title: {
                display: false,
                text: "Jumlah Data Absen Berdasarkan Status Absen"
            },
            legend: {
                display: true,
                position: "bottom"
